...
{
    int res = 0;
    res = boost::xtime_get(&xt, boost::TIME_RCF_UTC);
    assert(res == boost::TIME_RCF_UTC);
...
inline void to_timespec_duration(const boost::xtime& xt, timespec& ts)
{
    boost::xtime cur;
    int res = 0;
    res = boost::xtime_get(&cur, boost::TIME_RCF_UTC);
    assert(res == boost::TIME_RCF_UTC);
...
inline void to_microduration(const boost::xtime& xt, int& microseconds)
{
    ...
    if (boost::xtime_get(&cur, boost::TIME_RCF_UTC) <= 0)
        microseconds = 0;
    else
    {
